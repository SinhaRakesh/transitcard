...
class View_Suggestion extends \View{
	...
	function init(){
		...
		$model = $this->add('rakesh\apartment\Model_Suggestion');
		...
		$v->add('H4')->set('Add Your Seggestion');

		$form = $col1->add('Form');
		$form->setModel($model,['title','description']);
		$form->addSubmit('Submit Your Suggestion')->addClass('btn btn-warning');
	
		$v = $col2->add('View_Info')->addClass('callout callout-info');
		$v->add('H4')->set('Your Seggestion History');
		$grid = $col2->add('Grid');
